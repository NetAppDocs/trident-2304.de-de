...
Trident enthält außerdem die `cloneFromPVC` Und `splitOnClone` Anmerkungen zum Erstellen von Klonen. Sie können diese Anmerkungen zum Klonen einer PVC verwenden, ohne die CSI-Implementierung (für Kubernetes 1.13 und frühere Versionen) verwenden zu müssen oder wenn Ihre Kubernetes-Version keine Beta-Volume-Snapshots unterstützt (Kubernetes 1.16 und frühere Versionen). Beachten Sie, dass Trident 19.10 den CSI-Workflow zum Klonen von einer PVC unterstützt.
